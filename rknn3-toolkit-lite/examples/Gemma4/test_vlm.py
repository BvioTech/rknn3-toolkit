...
prompt_postfix = "<turn|>\n<|turn>model\n"
DEFAULT_PROMPT = '<image>请描述这张图片。'

# Gemma4 image token 配置，和 C/C++ 侧常用探测 token 保持一致。
IMAGE_START = b"<|image>"
IMAGE_END = b"<image|>"
...
def prepare_vision_input(image_path, input_attr):
    ...
    if ori_img is None:
        print(f"Read image failed: {image_path}")
        exit(-1)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--rknn_vision_path", type=str, help="vision rknn model path", required=False, default=RKNN_VISION_MODEL)
    parser.add_argument("--rknn_llm_path", type=str, help="llm rknn model path", required=False, default=RKNN_LLM_MODEL)
    ...
    parser.add_argument("--image_path", type=str, help="input image path", required=False, default=IMAGE_PATH)
    ...
    try:
        ...
        feature, img_w, img_h = prepare_vision_input(args.image_path, vis_input_attr)
        ...
        # 输出通常为 [64,1536]，补成 [1,64,1536] 传给 RKNN3Image。
        if len(outputs.shape) == 2:
            outputs = np.expand_dims(outputs, 0)
        outputs = np.ascontiguousarray(outputs.astype(np.float16, copy=False))
